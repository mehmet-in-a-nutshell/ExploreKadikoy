...
export const metadata = {
    title: 'Kadıköy Oyun Gecesi Etkinlikleri - ExploreKadikoy',
    description: 'Kadıköy\'deki masa oyunları, kutu oyunları ve oyun gecelerini keşfedin.',
};
...
export default async function OyunGecesiPage() {
    ...
    const allCategoryEvents = distinctEvents
        .filter((e: any) => e.event_subtype === 'Oyun gecesi')
        .map((e: any) => ({
            id: e.id,
            title: e.title,
            slug: e.slug,
        ...
        <main className={styles.main}>
            <header className={styles.header}>
                <div className={styles.headerContent}>
                    <h1 className={styles.title}>Oyun Geceleri</h1>
                    <p className={styles.subtitle}>Eğlenceli zeka oyunları ve yeni insanlarla tanışma fırsatı.</p>
                </div>
            </header>
            ...
            {pastEvents.length > 0 && (
                <section className={styles.results} style={{ marginTop: '3rem', borderTop: '1px solid var(--border-color)', paddingTop: '3rem' }}>
                    ...
                    <div className={styles.resultsInfo} style={{ marginTop: '1.5rem' }}>
                        <p><span>{pastEvents.length} oyun gecesi</span> bulundu</p>
                    </div>
